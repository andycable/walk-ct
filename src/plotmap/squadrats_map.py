...
def build_heatmap_overlay(path=DISTANCE_CSV):
    """The coverage heatmap as a data URI, for L.imageOverlay.

    Returns {"url": data-uri, "bounds": [[s, w], [n, e]]}, or None when the
    distance CSV is missing.

    The raster itself is built in heatmap_overlay, shared with nearest_map so
    the two pages cannot disagree about where the colours sit. It is inlined
    HERE and written as a file THERE for a reason that belongs to this page:
    this one opens by double-clicking, off file://, where the browser will not
    fetch a sibling PNG. See that module's docstring.
    """
    overlay = heatmap_overlay.build_overlay(path)
    if overlay is None:
        return None
    return {
        "url": heatmap_overlay.png_data_uri(overlay["png"]),
